...
**3 new facial expressions** (face-atlas swaps, generated by
`_archive/make_faces_v5.py`, same technique as the existing blink/happy/sad):

- **surprise** 😲 — wide round shocked eyes + highlights. Fires on shake and on
  a loud shout into the mic.
- **love** 😍 — pink heart eyes. Fires when you smile at the front camera, on a
  10+ tap streak, and at each bond-stage moment.
- **cry** 🥲 — big glossy teary "委屈" eyes with a welling droplet. Fires when the
  cat is neglected (mood need) or when it's ignored after asking a question.

`setExpression` gains the three, each with a graceful fallback if its texture
